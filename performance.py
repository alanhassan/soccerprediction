...
if safest_bet.empty:
    ...
    safest_bet['Winning_bet_final'] =  safest_bet.apply(lambda x: 'Odds_H_X' if (x['Winning_bet'] >= 1.60 and x['Pred_H'] > x['Pred_A'] and x['Winning_prob'] < 0.85) else ('Odds_X_A' if (x['Winning_bet'] >= 1.60 and x['Pred_A'] > x['Pred_H'] and x['Winning_prob'] < 0.85) else x['Winning_bet']),
    axis=1
)
